...
public class SeedlingOfferServicelmpl implements SeedlingOfferService {
    ...
    @Override
    public <T> T querySeedlingOfferService(String page) {
        //判断page start
        if (page == null || page == "") {
        ...
        try {
            data = this.dao.querySeedlingOfferDao(arg);
        } catch (Exception e) {
    ...
    public Status addSeedlingOfferService(MultipartFile[] files) {
        ...
        int arg = this.fileService.fileUploadService(files, "offer", uuid);
        switch (arg) {
            case 604:
